...
그리고 `MutableList` 의 구현체가 지금은 `ArrayList` 이지만 미래에 이보다 더 나은 다양한 구현 클래스가 제공될 수 있으므로 인터페이스 리스트를 이용하는 것이 미래 변동 사항에 유연하게 대처할 수 있다는 의견도 있습니다. 

결국 동작 수행은 똑같으므로 본인의 **개발 가치관에 따라서 자유롭게 선택하거나 직접 구현해서 사용**하면 될 것 같습니다. 직접 구현할 경우, 코틀린에서 사용자의 구현을 돕기 위해 뼈대가 잡혀있는 `AbstractionCollection` , `AbstractList`  등의 추상 클래스를 지원하고 있습니다. 참고하시면 좋을 것 같습니다.
